...
def benchmark_config(
    ...
    def prepare(i: int):
        ...
        if backend == "triton":
            return
        gmm1_ptrs, gmm2_ptrs, batch_sizes = prepare_gmm_fused_experts(
            x, w1, w2, hs_tensors_buffer, intermediate_cache1_buffer, intermediate_cache2_buffer, intermediate_cache3_buffer, topk_weights_buffer, topk_indices_buffer)
    ...
    def run():
        if backend == "triton":
            ...
            gmm_fused_experts_no_sum(
                x,
                hs_tensors_buffer,
                w1t if backend == "cutlass" else w1,
                w2,
    ...
    for i in range(num_iters):
        # use same data each iter otherwise cublas grouped gemm
        # cannot be correctly captured with cuda graph
        prepare(0)
        torch.cuda.synchronize()
    ...
    if args.tune:
        ...
        configs = _distribute(
            "tune", [(batch_size, E, shard_intermediate_size, hidden_size,
                      ...
                      args.backend, not args.no_cuda_graph)
