...
def find_best_tokens_across_batchsize(expert_selection_per_node: List[Tuple[int, ...]],
                                      k_experts_per_token:int) -> Tuple[int, int]:
    candidate_bs = [1, 2, 4, 8] + list(range(16, len(expert_selection_per_node) + 1, 16))
    best_ratio = float("inf")
    best_exp = None
    best_bs = None
    for batch_size in candidate_bs:
        activated_experts = find_best_tokens_and_experts(expert_selection_per_node, k_experts_per_token, batch_size)
        print(f"batch size: {batch_size}, activated experts: {activated_experts}, ratio: {activated_experts / batch_size}")
        if activated_experts / batch_size < best_ratio:
            best_ratio = activated_experts / batch_size
            best_exp = activated_experts
            best_bs = batch_size
    return best_exp, best_bs
...
def calculate_distribution(n_samples: int,
    ...
    for _ in tqdm.trange(n_trials):
        ...
        if solver == "gurobi":
            activated_experts, best_bs = find_best_tokens_across_batchsize(expert_selection_per_node, k_experts_each)
        else:
        ...
        results.append((activated_experts, best_bs))
    return results
...
def main():
    ...
    results = calculate_distribution(n_samples, k_experts_each, k_candidate_experts, n_experts, args.solver)
    exit(0)
    path = (f"dist_bs_ne{n_experts}_k{k_experts_each}"
    ...
    if not os.path.exists(path):
        with open(path, "w") as f:
            f.write("Activated Experts,Batch Size,Samples\n")
    with open(path, "a") as f:
        for exp, bs in results:
            f.write("{},{},{}\n".format(exp, bs, n_samples))
