...
def find_best_tokens(current_token_nodes: List[List[GraphNode]],
                   max_batch_size: int,
                   approx_bound=1) -> List[int]:
    ...
    with gp.Env(empty=True) as env:
        ...
        with gp.Model(env=env) as model:
            x = model.addMVar(len(current_token_nodes), vtype=gp.GRB.CONTINUOUS)
            ...
            return best_req_indices, activated_experts
...
class PrioritizeThroughputLayerwise(ScheduleStrategy):
    ...
    def schedule(self,
        ...
        if not self._current_req_ids:
            ...
            def get_random_selection(nodes):
                import random
                k_nodes = random.sample(nodes, max_batch_size)
                # concat k_nodes
                k_nodes = [node for nodes in k_nodes for node in nodes]
                act_exps = len(_c_act_exps(k_nodes))
                return act_exps
            import code
            code.interact(local=locals())
