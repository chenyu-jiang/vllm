...
def create_model(args, weights):
    mlp_model = MixtralExpertMLP.from_weights(*weights, _to_torch_dtype(args.dtype))
    mlp_model = mlp_model.to(args.device)
    return mlp_model

def calc_norm_diff_after_rank_reduction(mlp_model, eval_loader, ranks):
    mlp_model.eval()
    all_outputs = {
        "w1": [],
        "w3": [],
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(eval_loader, total=len(eval_loader)):
            inputs = batch
            w1_out, w3_out, w2_out = mlp_model(inputs)
            all_outputs["w1"].append(w1_out.detach())
            all_outputs["w3"].append(w3_out.detach())
            all_outputs["w2"].append(w2_out.detach())
...
def main_func(args):
    ...
    # create model
    mlp_model = create_model(args, weights)
    # calculate norm diff
    ranks = [8, 16, 32, 64, 128, 256, 512] + list(range(1024, 4096 + 512, 512))
    norm_diffs, norm_diff_percents, mean_diffs, max_diffs, explained_vars = calc_norm_diff_after_rank_reduction(mlp_model, data_loader, ranks)
    # save results
    exists = os.path.exists(args.output_path)
    with open(args.output_path, "a") as f:
        if not exists:
            f.write("weight,expert_id,layer_id,rank,norm_diff,norm_lost_percent,mean_diff,max_diff,explained_var\n")
        for w_name in ["w1", "w3", "w2"]:
            for rank, norm_diff, norm_diff_percent, mean_diff, max_diff, explained_var in zip(ranks, norm_diffs[w_name], norm_diff_percents[w_name], mean_diffs[w_name], max_diffs[w_name], explained_vars[w_name]):
                f.write(f"{w_name},{args.expert_id},{args.layer_id},{rank},{norm_diff},{1 - norm_diff_percent},{mean_diff},{max_diff},{explained_var}\n")
