...
def calc_norm_diff_after_rank_reduction(w1, w2, w3, ranks, dtype, device):
    ...
    all_outputs = {"w1": to_torch_dtype(w1, dtype, device),
                   "w2": to_torch_dtype(w2, dtype, device),
                     "w3": to_torch_dtype(w3, dtype, device)}
...
def main_func(args):
    ...
    norm_diffs, norm_diff_percents, mean_diffs, max_diffs, explained_vars = calc_norm_diff_after_rank_reduction(*weights, ranks, args.dtype, args.device)
    # save results
