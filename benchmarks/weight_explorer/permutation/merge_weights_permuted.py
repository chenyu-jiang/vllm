...
def train(weights: List[torch.Tensor], xs: List[torch.Tensor], alpha=1.0):
...
def main_func(args, save=True):
    ...
    for expert_id in args.experts_to_merge:
        if expert_id == 0:
            ...
            # load permuted
            w1, _, w3 = torch.load(os.path.join(args.permuted_dir, f"l{args.layer_id}_e{expert_id}", "permuted_weights.pt"))
            if args.weight_id == 1:
                weights.append(w1.to(args.device).float())
            elif args.weight_id == 3:
                weights.append(w3.to(args.device).float())
    # load data
    data = create_dataloaders(args)
    # train
    merged_weight = train(weights, data, alpha=args.alpha)
    # save
    if save:
