...
def solve_qp(
    neuron_to_replace: torch.Tensor,  # shape (1, d) or (b, d)
    ...
    if len(neurons_to_perturb.shape) == 1:
        neuron_to_replace = neuron_to_replace.unsqueeze(0)

    assert neurons_to_perturb.shape[1] == neuron_to_replace.shape[1]
    assert neurons_to_perturb.shape[0] == neuron_to_replace.shape[0] or neuron_to_replace.shape[0] == 1

    def objective(result_neuron):
        # (neuron_to_replace - result_neuron) @ x0 shape: (b, n)
        return (
            torch.norm((neuron_to_replace - result_neuron) @ x0, dim=-1) ** 2
        )

    def constraint(result_neuron):
        # (result_neuron - neurons_to_perturb) @ xi shape: (b, n)
        return (
            torch.norm((result_neuron - neurons_to_perturb) @ xi, dim=-1) ** 2
            - norm_limit
        )
...
def train(
    ...
    expert_ids = []
    min_perturbation_error = float("inf")
    ...
    for expert_id, this_expert_xs, this_expert_neurons in tqdm(
        ...
        for i in trange(n_batches, desc="Neuron Batch", leave=False):
            ...
            min_error, min_idx = perturbation_error.min(dim=0)
            if min_error < min_perturbation_error:
                min_perturbation_error = min_error
...
def main_func(args, save=True):
    ...
    w1 = exper_id_to_params[args.expert_id]["w1"].to(args.device).bfloat16()
    w2 = exper_id_to_params[args.expert_id]["w2"].to(args.device).bfloat16()
    w3 = exper_id_to_params[args.expert_id]["w3"].to(args.device).bfloat16()
    # load data
    data = create_dataloaders(args)
    data = data[args.expert_id]
    ...
    ).to(device=args.device).bfloat16()
    ...
    perturbed_w2 = torch.load(
        os.path.join(
            args.output_dir,
            get_output_subdir(args, override_weight_id=2),
            "results.pt",
        )
    )
    # test
    orig_output = (F.silu(data @ w1.t()) * (data @ w3.t())) @ w2.t()
    perturbed_output = (F.silu(data @ perturbed_w1.t()) * (data @ perturbed_w3.t())) @ perturbed_w2.t()
    norm_error = torch.norm(orig_output - perturbed_output, dim=-1).mean()
    print(f"Norm error: {norm_error.item()}")
